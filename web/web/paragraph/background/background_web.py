...
def judge(text,grade=None,num=None):
	...
	clf_s=pickle.load(open('/home/tfg2016/web/paragraph/background/clf_s.p','rb'))
	pr=clf_s.predict(sl)
	for ans in pr:
		dl[ans]+=1
	if grade != None:
	...
	a.extend(k)'''
	if grade != None:		
		fw=open('/home/tfg2016/web/paragraph/background/paragraph/%d/%d.judge'%(grade,num),'w')
		json.dump(a,fw)
		fw.close()
	clf_p=pickle.load(open('/home/tfg2016/web/paragraph/background/clf_p_9.p','rb'))
	pr=clf_p.predict([a])
	return pr[0],dl
